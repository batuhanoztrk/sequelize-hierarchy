...
const Support = {
	...
	prepareTransactionTest(sequelize, callback) {
		...
		if (dialect === "sqlite") {
			...
			return new bluebird.Promise((resolve) => {
				...
				if (callback) {
					_sequelize.sync({ force: true }).success(() => {
						callback(_sequelize);
					});
				} else {
	...
	clearDatabase(sequelize) {
		return sequelize.drop().then(() => {
			...
			if (sequelize.modelManager.models)
	...
	getSupportedDialects() {
		return fs
			.readdirSync(`${__dirname}/../node_modules/sequelize/lib/dialects`)
			.filter(
				(file) =>
					file.indexOf(".js") === -1 &&
					file.indexOf("abstract") === -1
			);
	},
	...
	getTestDialect() {
		...
		if (this.getSupportedDialects().indexOf(envDialect) === -1) {
			throw new Error(
				`The dialect you have passed is unknown. Did you really mean: ${envDialect}`
			);
		}
